...
public class ClickStreamTwo {
            ...
            try {
                ...
                for (int i = 0; i < beans.size(); i++) {
                    ...
                    if (timeDiff < 30 * 60 * 1000) {
                        v.set(beans.get(i - 1).getRemote_user() + "\001" + beans.get(i - 1).getTime_local() + "\001" + beans.get(i - 1).getRequest() + "\001" + step + "\001" + (timeDiff / 1000) + "\001" + beans.get(i - 1).getHttp_referer() + "\001"
                        ...
                        v.set(beans.get(i - 1).getRemote_user() + "\001" + beans.get(i - 1).getTime_local() + "\001" + beans.get(i - 1).getRequest() + "\001" + (step) + "\001" + (60) + "\001" + beans.get(i - 1).getHttp_referer() + "\001"
                                + beans.get(i - 1).getHttp_user_agent() + "\001" + beans.get(i - 1).getBody_bytes_sent() + "\001" + beans.get(i - 1).getStatus());
                        context.write(key, v);
                        // 输出完上一条之后，重置step编号
                        step = 1;
                    ...
                    if (i == beans.size() - 1) {
                        // 设置默认停留市场为60s
                        v.set(bean.getRemote_user() + "\001" + bean.getTime_local() + "\001" + bean.getRequest() + "\001" + step + "\001" + (60) + "\001" + bean.getHttp_referer() + "\001" + bean.getHttp_user_agent() + "\001" + bean.getBody_bytes_sent() + "\001" + bean.getStatus());
                        context.write(key, v);
                    }
                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(ClickStreamTwo.class);
...
//		FileInputFormat.setInputPaths(job, new Path("c:/weblog/18valid"));
